...
def printUsageHelp():
	...
	print ("\t-D or --displayMap = This creates two files: one with the marker metadata appended with the map data of the selected mapset (marker_filename.ext) and a mapset metadata file. The mapset metadata filename is fetched from the value passed to  the -b (--mapsetOutputFile) parameter, if the -b parameter is not set, it defaults to marker_filename.mapset.")
